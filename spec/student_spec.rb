...
describe "Student" do
 ...
 :bio=>
  ...
  let!(:student) {Student.new({:name=>"Alex Patriquin", :location=>"New York, NY", :profile_url => "www.learn.co/notarealperson"})}
  ...
  describe "#new" do
    it "takes in an argument of a hash and sets that new student's attributes using the key/value pairs of that hash." do
      ...
      expect(student.profile_url).to eq("www.learn.co/notarealperson")
    end
