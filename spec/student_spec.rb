...
puts    it "adds that new student to the Student class' collection of all existing students, stored in the `@@all` class variable." do
      expect(Student.class_variable_get(:@@all).first.name).to eq("Alex Patriquin")
    end
  end

puts  describe ".create_from_collection" do
  ...
  puts describe "#add_student_attributes" do
