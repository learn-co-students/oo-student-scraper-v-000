...
class Scraper
  ...
  def self.scrape_index_page(index_url)
    ...
    student.each do |card|
      ...
      @array_of_hashes << {:name => name, :location => location, :profile_url => "http://127.0.0.1:4000/#{profile_url}"}
    end
    @array_of_hashes
  end
  ...
  def self.scrape_profile_page(profile_url)
    ...
    social.children.css("a").each do |x|
      if x.attribute("href").to_s.include?("twitter") == true
        ...
        profile_hash[:blog] = x.attribute("href").to_s
      end
    end
    profile_hash[:profile_quote] = html.css(".profile-quote").text
    profile_hash[:bio] = html.css(".description-holder p").text
    #have to have 2 terminal tabs open and run jekyll serve
    #above twitter should get all "a"'s no?
