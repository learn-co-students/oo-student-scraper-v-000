...
class Scraper
  ...
  def self.scrape_profile_page(profile_url)
    ...
    social_media = profile.css("div.social-icon-container").css("a").collect do |account|
      account.attribute("href").value
    end

    fred = social_media.each do |link|
    ...
    student[:bio] = profile.css("div.description-holder").css("p").text if profile.css("div.description-holder").css("p")
