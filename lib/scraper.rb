...
class Scraper
  ...
  def self.scrape_index_page(index_url) #takes url of inex page and will scrape it
    students = Nokogiri::HTML(open(index_url)) #sets the nokogiri objects ruturned from the index url to a var
    students_array = [] #intializes an array to save scraped data
    students.css('div.roster-cards-container').each do |individual_students| #takes noko var and returns css div then iterates with .each 
      individual_students.css('.student-card a').each do |student| #takes passed results from 
